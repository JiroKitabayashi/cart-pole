...
def get_path(dir_name):
    ...
    dir_list = sorted([int(x) for x in dir_list if x != '.DS_Store'], reverse=True)
    if not dir_list:
        return 1
    dir_num = dir_list[0]
    return dir_num + 1
...
def main():
    ...
    # 実験結果収集
    res_dir = 'log/'
    exp_num = get_path(res_dir)
    log_dir = res_dir + str(exp_num) + '/'
    os.makedirs(log_dir)

    f = open(log_dir + 'result.csv' , 'w')
    writer = csv.writer(f, lineterminator='\n')
    writer.writerow(['episode', 'last_step', 'rewards_mean'])
    # [5] メインルーチン--------------------------------------------------
    for episode in range(num_episodes):  #試行数分繰り返す
        # 環境の初期化
        observation = env.reset()
        state = digitize_state(observation, num_dizitized)
        action = np.argmax(q_table[state]) # 報酬が大きい方のインデックスを返す 0か１
        episode_reward = 0
        ...
        for t in range(max_number_of_steps):  #1試行のループ
            ...
            if done:
                ...
                writer.writerow([episode, t + 1, total_reward_vec.mean()])
        ...
        if (total_reward_vec.mean() >=
            ...
            islearned = 1
            np.savetxt(log_dir + 'learned_Q_table.csv',q_table, delimiter=',') #Qtableの保存する場合
            if isrender == 0:
                env = gym.wrappers.Monitor(env, log_dir + './vid/'+ str(episode) + '/', video_callable=lambda episode_id: True,force=True) #動画保存する場合
                isrender = 1
                break
        # nエピソードおきの挙動を録画する
        if (episode % 100) == 0:
    ...
    f.close()
    if islearned:
        np.savetxt(log_dir + 'final_x.csv', final_x, delimiter=',')
